/**
 * fetchMovies.test.js tests many edge cases when fetching data from the API and matching it with the dataset.
...
describe("GET/ Request test at endpoint '/oneMovie/fetchMovieDataFromApi/'", () => {
     ...
     * NOTES: Edge case type 2 - Movie's original title of API doesn't match with movie title of dataset
     */
    test("Tests endpoint with 'Mantis Fist Fighter - 1980'", async () => {
        const response = await request(app).get("/api/oneMovie/fetchMovieDataFromApi").query({ title: "Mantis Fist Fighter", year: 1980})
        .expect("Content-Type", /json/)
        .expect(200);
    ...
    /**
     * Venom - 1981
     * NOTES: Edge case type 3 - Query returns multiple movies with very similar release years. Return right movie with Venom as title
     */
     test("Tests endpoint with 'Venom - 1981'", async () => {
        const response = await request(app).get("/api/oneMovie/fetchMovieDataFromApi").query({ title: "Venom", year: 1981})
        .expect("Content-Type", /json/)
        .expect(200);
    ...
    /**
     * Inchon - 1981
     * NOTES: Edge case type 4 - Query returns wrong movie with different movie title and no description. Ambiguity on which movie is the correct one.
     */
     test("Tests endpoint with 'Inchon - 1981'", async () => {
        const response = await request(app).get("/api/oneMovie/fetchMovieDataFromApi").query({ title: "Inchon", year: 1981})
        .expect("Content-Type", /json/)
        .expect(200);
        ...
        // const expectedMovie = {
        //     title: "Inchon",
        //     description: "A noisy and absurd re-telling of the great 1950 invasion of Inchon during the Korean War which was masterminded by General Douglas MacArthur.",
    ...
    /**
     * The Entity - 1982
     * NOTES: Edge case type 3 - Query returns multiple movies with very similar release years. Return right movie with The Entity as title
     */
     test("Tests endpoint with 'The Entity - 1982'", async () => {
        const response = await request(app).get("/api/oneMovie/fetchMovieDataFromApi").query({ title: "The Entity", year: 1982})
        .expect("Content-Type", /json/)
        .expect(200);
